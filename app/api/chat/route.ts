...
async function askExtractorQuestion(mediaId: string, markdown: string, question: string) {
  ...
  try {
    logInfo("chat_request_started", { mediaId, metadataQueue: METADATA_QUEUE, requestId });
    ...
    const { consumerTag } = await channel.consume(
      ...
      (msg) => {
        ...
        resolveResponse?.(msg.content.toString("utf-8"));
      },
      { noAck: true },
    );

    const timeout = setTimeout(() => {
      rejectResponse?.(new Error("Extractor did not answer in time"));
    }, CHAT_TIMEOUT_MS);
    ...
    const raw = await responsePromise.finally(async () => {
  ...
  try {
    ...
    if (entry.status !== "completed") {
      return NextResponse.json(
        { error: "Document is not ready for questions", status: entry.status },
        { status: 409 },
      );
    }
