...
function buildCardPrompt(docType: string | null, filename: string | null, requestId: string): { systemPrompt: string; prompt: string } {
  ...
  const prompt = [
    ...
    "Для счета или счета-фактуры: номер, дата, продавец или поставщик, покупатель или плательщик, получатель, ИНН/КПП, банковские реквизиты, БИК, расчётный счёт, корреспондентский счёт, суммы без НДС, НДС, итоговую сумму, основание и назначение платежа.",
